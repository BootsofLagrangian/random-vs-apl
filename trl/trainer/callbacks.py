...
def _generate_completions(
    ...
    with unwrap_model_for_generation(model, accelerator) as unwrapped_model:
        for idx in tqdm(range(0, len(prompts), batch_size)):
            ...
            '''
                The batch is a structure that contains a list within a list, and a dict within that. 
                It allows flattening and string extraction so that it can be used as tokenizer input.
                # Flatten only if batch is not one of str, List[str], or List[List[str]]
            '''
            if not (
                isinstance(batch, str) or
                (isinstance(batch, list) and all(isinstance(x, str) for x in batch)) or
                (isinstance(batch, list) and all(isinstance(x, list) and all(isinstance(y, str) for y in x) for x in batch))
            ):
                batch = [x[0]["content"] for x in batch]
            tokenized_batch = tokenizer(batch, return_tensors="pt", padding=True, truncation=True).to(model.device)
            generations = unwrapped_model.generate(
                **tokenized_batch,
                generation_config=generation_config,
            )
            completion_ids = generations[:, tokenized_batch['input_ids'].size(1) :]
            completion_ids, _ = truncate_right(completion_ids, eos_token_id, pad_token_id)
...
class WinRateCallback(TrainerCallback):
    ...
    def __init__(
        ...
        if self.trainer.eval_dataset is None:
            ...
            '''
                Add exception because on_train_begin uses the "prompt" column of the evaluation dataset
            '''
            if "prompt" not in self.trainer.eval_dataset.column_names:
                raise ValueError("You must provide an evaluation dataset with a 'prompt' column.")
            
            self.eval_dataset = self.trainer.eval_dataset

        if num_prompts is not None:
            if num_prompts > len(self.eval_dataset):
                raise ValueError("The num_prompts value exceeds the dataset size.")
    ...
    def _get_completions(
        ...
        if args.use_vllm:
            _, _, completion_ids, _ = self.trainer._generate_vllm(
                self.trainer.model_wrapped, prompts
            )
    ...
    def on_train_begin(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs):
        ...
        accelerator.print("[WinRateCallback] Creating reference completions...")
        prompts = self.eval_dataset["prompt"]
        # with accelerator.split_between_processes(self.eval_dataset["prompt"]) as prompts:
        prompts = prompts[accelerator.process_index::accelerator.num_processes]  # Ensure each process gets a unique set of prompts

        completions = self._get_completions(
            args, prompts, tokenizer, accelerator
        )
        completions = gather_object(completions)
        self.ref_completions = completions
        ...
        accelerator.print("[WinRateCallback] - Number of completions:", len(completions))
    ...
    def on_evaluate(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs):
        ...
        # Proactively release cached GPU memory between eval rounds to reduce
        # fragmentation and lower the chance of OOMs during long runs.
        try:
            torch.cuda.empty_cache()
        except Exception:
            pass

class UnbiasedWinRateCallback(WinRateCallback):
    def on_evaluate(self, args: TrainingArguments, state: TrainerState, control: TrainerControl, **kwargs):
        tokenizer = kwargs["processing_class"]
        tokenizer.padding_side = "left"
        accelerator = self.trainer.accelerator
        ...
        with accelerator.split_between_processes(self.eval_dataset["prompt"]) as prompts:
            completions_model = _generate_completions(
                prompts,
                model=model,
                tokenizer=tokenizer,
                accelerator=accelerator,
            ...
            # 양방향 (ref, model) and (model, ref)
            completions_fwd = list(zip(self.ref_completions, completions_model))   # (ref, model)
            completions_bwd = list(zip(completions_model, self.ref_completions))   # (model, ref)
            prompts = prompts * 2
            completions = completions_fwd + completions_bwd

            if self.use_soft_judge:
                ref_win_probs = self.judge.judge(prompts, completions, self.shuffle_order, return_scores=True)
                winner_indices = [0 if p > 0.5 else 1 for p in ref_win_probs]
                ref_win_probs = gather_object(ref_win_probs)
            else:
                winner_indices = self.judge.judge(prompts, completions, self.shuffle_order)
        ...
        # Logging (unbiased winrate = average over both directions)
        if self.trainer.accelerator.is_main_process:
            win_rate = sum(winner_idx == 1 for winner_idx in winner_indices) / len(winner_indices)
            if self.use_soft_judge:
                avg_win_prob = 1.0 - sum(ref_win_probs) / len(ref_win_probs)
            ...
            if "wandb" in args.report_to:
                ...
                if wandb.run is not None:
                    ...
                    wandb.log({"win_rate_completions_unbiased": wandb.Table(dataframe=df)})
            ...
            if "comet_ml" in args.report_to:
                ...
                log_table_to_comet_experiment(
                    name="win_rate_completions_unbiased.csv",
                    table=df,
                )

        # Same reasoning as in WinRateCallback: clear CUDA cache after each eval
        # pass to keep memory fragmentation under control.
        try:
            torch.cuda.empty_cache()
        except Exception:
            pass

class QueryStrategyCallback(TrainerCallback):
    def __init__(self, training_args, active_args, strategy):
        self.training_args = training_args
        self.strategy = strategy
        
        batch_size = (training_args.per_device_train_batch_size * strategy.trainer.accelerator.num_processes * training_args.gradient_accumulation_steps)
        self.query_freq = int(active_args.num_query / batch_size * active_args.updates_per_sample)
        self.active_args = active_args
        assert self.query_freq >= 1, "Query frequency: {query_freq} should be > 1"
        
    def on_step_end(self, args, state, control, **kwargs):
        if state.global_step > 0 and (state.global_step + 1) % self.query_freq == 0:
            self.strategy.init_labeled_indice()
            
            args_to_update = self.strategy.query(self.active_args.num_query)
            self.strategy.update_data(args_to_update)
