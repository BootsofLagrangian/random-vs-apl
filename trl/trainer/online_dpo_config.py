...
class OnlineDPOConfig(TrainingArguments):
    ...
    Parameters:
        ...
        eval_judge (`str` or `None`, *optional*, defaults to `None`):
            Name of the judge to use for evaluation win rate computation. Separate from the training judge.
    ...
    eval_judge: Optional[str] = field(
        default=None,
        metadata={
            "help": "Name of the judge to use for evaluation win rate computation. Separate from the training judge."
        },
    )
    num_eval_prompts: int = field(
        default=1000,
        metadata={"help": "The number of prompts for win-rate evaluation."},
    )
    ...
    simpo_gamma: Optional[float] = field(
        default=1.0,
        metadata={
            "help": "Parameter for the SIMPO loss. Must be non-negative and is required if `loss_type` is `simpo`.",
        },
    )
    ...
    stop_strings: Optional[str] = field(
        default=None,
        metadata={
            "help": "Comma-separated stop strings to stop generation (e.g., 'user:, user:, user, user'). "
            "If not provided, defaults to common user-turn markers."
        },
    )
    ...
    reward_batch_size: int = field(
        default=2,
        metadata={
            "help": "Batch size used when scoring the reward model or judge. Smaller values reduce GPU memory usage."
        },
    )
