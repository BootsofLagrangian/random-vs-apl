...
@torch.no_grad()
def prepare_weights_for_vllm(model: torch.nn.Module) -> torch.nn.Module:
    from copy import deepcopy
    model_copy = deepcopy(model)
    if is_peft_available():
        if isinstance(model, PeftModel):
            model_copy = model_copy.merge_and_unload()
    model_copy: torch.nn.Module = model_copy.dequantize()
...
def get_reward(
    ...
    # Avoid materializing all hidden states (very memory-heavy for long sequences).
    output = lm_backbone(
        input_ids=input_ids,
        attention_mask=attention_mask,
    ...
    if last_hidden_state is None:
        # Fallback for non-standard backbones.
        output = lm_backbone(
            input_ids=input_ids,
            attention_mask=attention_mask,
            position_ids=position_ids,
    ...
    reward_logits = model.score(last_hidden_state)
...
def compute_reward_scores(
    ...
    *,
    device: torch.device,
    prompt_completion_ids: Optional[torch.Tensor] = None,
    context_length: Optional[int] = None,
    contain_eos_mask: Optional[torch.Tensor] = None,
    missing_eos_penalty: Optional[float] = None,
    reward_batch_size: int = 2,
    force_sequence_classification: Optional[bool] = None,
) -> torch.Tensor:
    """
    Compute scalar reward scores for (prompt, completion) pairs using either a sequence
    classification head or an LM-style reward head.

    This helper centralizes reward-model inference so that training and evaluation
    (e.g. `OnlineDPOTrainer` and `RMWrappedJudge`) stay consistent across
    different reward model architectures.
    """
    from transformers import DebertaV2ForSequenceClassification

    model_name_or_path = getattr(getattr(reward_model, "config", None), "_name_or_path", "") or ""
    is_nemotron_reward = "Nemotron" in model_name_or_path and "Reward" in model_name_or_path

    # Decide whether to treat the model as a sequence classification RM.
    use_seq_cls = force_sequence_classification
    if use_seq_cls is None:
        # If the model exposes a `.score(hidden_states)` head, use the LM-based reward path.
        # Otherwise, treat it as a sequence-classification RM returning scalar logits.
        use_seq_cls = not hasattr(reward_model, "score")

    if use_seq_cls:
        # Ensure pad_token_id exists for batch scoring.
        # Many decoder-only sequence classification heads error for batch_size>1 if pad_token_id is unset.
        if reward_tokenizer.pad_token_id is None:
            if getattr(reward_tokenizer, "eos_token_id", None) is not None:
                reward_tokenizer.pad_token = reward_tokenizer.eos_token
            elif getattr(reward_tokenizer, "unk_token_id", None) is not None:
                reward_tokenizer.pad_token = reward_tokenizer.unk_token
        if getattr(getattr(reward_model, "config", None), "pad_token_id", None) is None and reward_tokenizer.pad_token_id is not None:
            reward_model.config.pad_token_id = reward_tokenizer.pad_token_id

        # Nemotron reward models require chat-template formatting.
        if is_nemotron_reward and hasattr(reward_tokenizer, "apply_chat_template"):
            trailing_assistant = re.compile(r"\s*Assistant:\s*$", re.IGNORECASE)
            leading_assistant = re.compile(r"^\s*Assistant:\s*", re.IGNORECASE)

            def _clean_prompt(text: str) -> str:
                return trailing_assistant.sub("", str(text)).rstrip()

            def _clean_completion(text: str) -> str:
                return leading_assistant.sub("", str(text)).lstrip()

            texts: list[str] = []
            for p, c in zip(prompts, completions):
                user = f"{_clean_prompt(p)} /nothink"
                assistant = "<think>\n\n</think>\n\n" + _clean_completion(c)
                messages = [{"role": "user", "content": user}, {"role": "assistant", "content": assistant}]
                texts.append(reward_tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=False))

            # Left padding is safer for decoder-only sequence classification pooling.
            ...
            # Generic text-pair classification path (covers Deberta, Skywork Reward-V2, Beaver, etc.).
            encoded = reward_tokenizer(
                prompts,
                completions,
                padding=True,
        ...
        batch_sz = max(1, int(reward_batch_size))
        for start in range(0, input_ids.size(0), batch_sz):
            end = start + batch_sz
            ids_chunk = input_ids[start:end].to(device)
            if attention_mask is not None:
                mask_chunk = attention_mask[start:end].to(device)
            else:
                mask_chunk = None
            with torch.inference_mode():
                if mask_chunk is not None:
                    logits = reward_model(ids_chunk, attention_mask=mask_chunk).logits
                else:
                    logits = reward_model(ids_chunk).logits
            ...
            if logits.shape[1] == 1:
                ...
                # Convert 2-class output to scalar preference score.
                batch_scores = logits[:, 1] - logits[:, 0]
            else:
                raise ValueError(f"Expected num_labels in {{1,2}}, got logits shape {tuple(logits.shape)}")
            scores_chunks.append(batch_scores.detach().cpu())

        scores = torch.cat(scores_chunks, dim=0).to(device)
    else:
        # LM-style reward path using the hidden-state score head (e.g. Llama RM with `.score`).
        if prompt_completion_ids is None or context_length is None:
            raise ValueError(
                "prompt_completion_ids and context_length must be provided for LM-based reward models."
            )
        batch_sz = max(1, int(reward_batch_size))
        scores_chunks: list[torch.Tensor] = []
        for start in range(0, prompt_completion_ids.size(0), batch_sz):
            end = start + batch_sz
            _, chunk_scores, _ = get_reward(
                reward_model,
                prompt_completion_ids[start:end],
                reward_tokenizer.pad_token_id,
                context_length,
            )
            scores_chunks.append(chunk_scores.detach().cpu())
        scores = torch.cat(scores_chunks, dim=0).to(device)
        # Some decoder-only SeqCls heads yield 2-class logits; map to scalar.
        if scores.ndim == 2:
            if scores.shape[1] == 1:
                scores = scores[:, 0]
                ...
                raise ValueError(f"Expected num_labels in {{1,2}}, got scores shape {tuple(scores.shape)}")

    # Penalize completions that never emitted an EOS token if requested.
    if missing_eos_penalty is not None and contain_eos_mask is not None:
        scores = scores.clone()
        scores[~contain_eos_mask] -= missing_eos_penalty
...
def build_stop_token_sequences(
    tokenizer: PreTrainedTokenizerBase, stop_strings: list[str]
) -> list[list[int]]:
...
def truncate_on_stop_sequences(
    input_ids: torch.Tensor, stop_sequences: list[list[int]], pad_token_id: int
) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    for i, seq in enumerate(seqs):
        try:
            pad_idx = seq.index(pad_token_id)
        except ValueError:
            pad_idx = len(seq)
        best_idx = None
        for stop_seq in stop_sequences:
            stop_len = len(stop_seq)
            if stop_len == 0 or stop_len > pad_idx:
                continue
            for j in range(0, pad_idx - stop_len + 1):
                if seq[j : j + stop_len] == stop_seq:
                    if best_idx is None or j < best_idx:
                        best_idx = j
                    break
        if best_idx is not None:
            for j in range(best_idx, len(seq)):
                seq[j] = pad_token_id

    output_ids = torch.tensor(seqs, device=device, dtype=input_ids.dtype)
    mask = (output_ids != pad_token_id).long()
    return output_ids, mask
