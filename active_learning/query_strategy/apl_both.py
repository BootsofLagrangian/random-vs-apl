...
class APLBoth(Strategy):
    def __init__(self, trainer, system_prompt, config):
        super(APLBoth, self).__init__(trainer, system_prompt, config)
    ...
    def query(self, n):          
        ...
        local_entropies = []
        for i, inputs in enumerate(unlabeled_dataloader):
        ...
        topk_indices = torch.topk(total_entropies, k=len(subset_unlabeled_indices) // (2 * self.k), largest=True).indices.cpu()
        bottomk_indices = torch.topk(total_entropies, k=len(subset_unlabeled_indices) // (2 * self.k), largest=False).indices.cpu()
        topk_indices = torch.cat([topk_indices, bottomk_indices])
